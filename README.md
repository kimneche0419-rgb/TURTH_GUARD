...
* **4대 탐지 모듈 통합 SDK:** 아래 모듈을 단일 규격으로 통합 제공.
  * **📝 텍스트 고증 검증** — 한국사 텍스트의 AI 생성(Perplexity/Burstiness) 탐지 + **내장 한국사 사료 지식베이스(국사편찬위원회 연표 기반)** 와 한국어 위키백과·DuckDuckGo·Naver Search·Google Fact Check **외부 검색 증거 병렬 교차 검증** 기반 **역사적 정합성 검증** + 사건·인물 × 연도 주장의 **연표 교차 검증(오프라인 결정론)** + 현대 기기·대상이 역사 인물·시대와 동시 등장하는 **시대착오(Anachronism) 할루시네이션 탐지**.
  * **🖼️ ELA 이미지 합성 탐지** — Error Level Analysis 압축 왜곡 + FFT 주파수 노이즈(GAN/Diffusion 격자 아티팩트)로 위조·합성 역사 이미지 탐지.
  * **🎬 딥페이크 페이스 스왑 탐지(영상/정지영상)** — 안면 랜드마크 비대칭 + 프레임 간 temporal jitter로 페이스 스왑 탐지. **`th stream`으로 RTSP/HTTP 라이브 스트림·장문 영상·웹캠을 시간 청크 단위로 실시간 증분 분석**(CRITICAL 청크 조기 종료 지원).
...
| `th stream <파일/URL/캠>` | `.\th stream <파일/URL/캠>` | **라이브 스트리밍·장문 영상 실시간 청크 분석** | RTSP/HTTP 스트림 URL, 비디오 파일, 웹캠 인덱스(예: `0`)를 청크 단위로 즉시 검증 — 청크 완성 시마다 실시간 출력, CRITICAL 발견 시 조기 종료 (`--chunk-seconds <초>`, `--max-chunks <N>`, `--no-early-stop`, `-f text/json` 지원) |
...
텍스트 리포트는 **다각도 판별/분석**(사료 정합성·한국사 연표 KB·AI 생성 가능성·선동성·출처 신뢰도·시대착오 — 각도별 점수 막대 + 3단계 판정)과 신뢰도 게이지를 시각적으로 제공하며, 모든 리포트에 **지정학적 역사 왜곡이 허용되지 않는 이유**(영토·주권 분쟁 근거 오염, 집단 기억 오염, 외교 신뢰 붕괴, 가해·피해 관계 전도, 생성형 AI 증폭)가 `significance` 섹션으로 함께 전달됩니다. JSON 리포트는 `perspectives`(각도별 `name`/`score`/`verdict`/`detail` + `summary`)·`significance` 필드로 동일 데이터를 노출합니다.

```
========== Truth History Scan Report ==========
대상 파일: 임진왜란은 1920년에 발발했다
종합 신뢰도: 0.35 (HIGH RISK)
███████░░░░░░░░░░░░░ 35%
스캔 결과: 변조 및 허위 정보 의심

다각도 판별 (6/6각도 판별 참여): 2개 각도에서 왜곡·위변조 의심 — 사료 정합성, 한국사 연표 KB
 - ████░░░░░░░░░░░░░░░░ 0.20 의심 사료 정합성 — 외부 증거 7건 교차 검증 · 증거-주장 상충 발견
 - ░░░░░░░░░░░░░░░░░░░░ 0.00 의심 한국사 연표 KB — 연표 상충 『임진왜란』 1592년~1598년 사이 — 1920년 주장은 시대와 상충
 - ...

📌 지정학적으로 역사가 왜곡되어서는 안 되는 이유
 - 영토·주권 분쟁의 근거 오염: ...
 - 생성형 AI에 의한 왜곡 증폭: ...
```
...
* 📜 **[한국사 사료 지식베이스 구현 상세](docs/knowledge_base.md):** 국사편찬위원회 연표 기반 오프라인 고증 계층 — 사건/인물 × 연도 주장 교차 검증 및 `truthhistory-kb` 증거 소스
* 📡 **[스트리밍 실시간 청크 분석 구현 상세](docs/streaming_analyzer.md):** RTSP/HTTP 라이브 스트림·장문 영상·웹캠의 시간 청크 단위 증분 분석 아키텍처
